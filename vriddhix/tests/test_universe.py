...
def test_resolves_membership_as_of_a_past_date(service, index_code):
    result = service.members_as_of(index_code, date(2021, 6, 1))
...
def test_a_stock_that_left_the_index_still_appears_in_history(service, index_code):
    """This is the survivorship test. DELISTED was removed in 2023; a 2021
    backtest that cannot see it is measuring survivors only."""
    result = service.members_as_of(index_code, date(2021, 1, 1))
    assert "DELISTED" in result.symbols


def test_a_stock_that_left_is_absent_after_its_removal(service, index_code):
    result = service.members_as_of(index_code, date(2024, 1, 1))
    assert "DELISTED" not in result.symbols
    assert "INFY" in result.symbols


def test_membership_upper_bound_is_exclusive(service, index_code):
    """A stock removed on date D is not a member on D."""
    assert "DELISTED" in service.members_as_of(index_code, date(2023, 6, 30)).symbols
    assert "DELISTED" not in service.members_as_of(index_code, date(2023, 7, 1)).symbols


def test_a_stock_is_absent_before_it_joined(service, index_code):
    assert "INFY" not in service.members_as_of(index_code, date(2021, 1, 1)).symbols
    assert "INFY" in service.members_as_of(index_code, date(2022, 6, 1)).symbols


def test_current_members_excludes_removed_names(service, index_code):
    current = service.current_members(index_code)
...
def test_dates_before_any_history_fall_back_and_are_flagged(service, index_code):
    """The bias must travel with the numbers, not live in a footnote."""
    result = service.members_as_of(index_code, date(2005, 1, 1))
...
def test_fallback_can_be_refused(session, seeded, index_code):
    ...
    with pytest.raises(UniverseError, match="fallback is disabled"):
        strict.members_as_of(index_code, date(2005, 1, 1))


def test_membership_built_only_from_backfill_is_flagged(session, seeded, index_code):
...
def test_removing_a_member_closes_the_interval_rather_than_deleting(service, session, index_code):
    from vriddhix.db.models import UniverseMember

    before = session.query(UniverseMember).count()
    service.remove_member(index_code, "TCS", date(2025, 1, 1))
    after = session.query(UniverseMember).count()

    assert after == before, "the row is the evidence that TCS was a member"
    assert "TCS" in service.members_as_of(index_code, date(2024, 1, 1)).symbols
    assert "TCS" not in service.members_as_of(index_code, date(2025, 6, 1)).symbols


def test_adding_an_unknown_symbol_raises(service, index_code):
    with pytest.raises(UniverseError, match="unknown symbol"):
        service.add_member(index_code, "NOSUCH", date(2024, 1, 1))


def test_unknown_index_raises(service, index_code):
...
def test_snapshot_records_completeness(service, index_code):
    good = service.create_snapshot(index_code, date(2021, 6, 1), "NIFTY500_2021H1")
    assert good.is_complete is True
    assert good.member_count == 3

    synthetic = service.create_snapshot(index_code, date(2005, 1, 1), "NIFTY500_2005")
